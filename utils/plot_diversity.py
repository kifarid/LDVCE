...
#base_path = LDCE_sd_correct_seed_0
...
save_base_path = "/misc/lmbraid21/faridk/diversity"
...
for pth_file in tqdm(sorted(glob.glob(bucket_folder + "/*.pth")), leave=False):
    ...
    for seed in range(5):
        current_dir_path = base_path + f"seed_{seed}" + "/bucket_0_50/"
    ...
    image_across_seeds =get_concat_h(*image_across_seeds).save(out_filepath)
    counter += 1