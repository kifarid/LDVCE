...
python -m scripts.dvce --config-name=v8_stable_diffusion_default \
    data.batch_size=4 \
    output_dir=/misc/lmbraid21/faridk/LDCE_sd_default \
    sampler.classifier_lambda=3.95 \
    sampler.dist_lambda=1.2 \
    sampler.deg_cone_projection=50. \
    data.start_sample=$PBS_ARRAYID data.end_sample=$((PBS_ARRAYID+1)) > logs/LDCE_sd_default_${PBS_ARRAYID}.log 
