...
python -m scripts.dvce --config-name=composition \
    data.batch_size=4 \
    output_dir=/misc/lmbraid21/faridk/axiomatic/composition/${seed} \
    seed=$seed \
    sampler.guidance=projected \
    sampler.classifier_lambda=3.95 \
    sampler.dist_lambda=1.2 \
    sampler.cone_projection_type=zero_binning \
    sampler.deg_cone_projection=50. > logs/composition_${seed}.log
