...
python -m scripts.dvce --config-name=v8_robust\
    data.batch_size=4 \
    strength=0.382 \
    output_dir=/misc/lmbraid21/faridk/ldvce_robust \
    data.start_sample=$PBS_ARRAYID data.end_sample=$((PBS_ARRAYID+1)) > logs/ldvce_robust_$PBS_ARRAYID.log 
